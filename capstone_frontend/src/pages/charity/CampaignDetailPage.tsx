...
import { Edit, Pause, Play, StopCircle, Download, AlertCircle, FileText, CreditCard, Wallet } from "lucide-react";
                    ...
                    {campaign.donation_channels.map((channel: any) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm truncate">{channel.label}</p>
                          <p className="text-xs text-muted-foreground capitalize">{channel.type}</p>
                          {channel.account_name && (
                            <p className="text-xs text-muted-foreground mt-1">{channel.account_name}</p>
                          )}
                          {channel.account_number && (
                            <p className="text-xs font-mono mt-1">{channel.account_number}</p>
                          )}
                          <Badge
                            variant="outline"
                            className={`mt-2 ${channel.is_active ? 'bg-green-500/10 text-green-600' : 'bg-gray-500/10 text-gray-600'}`}
                          >
